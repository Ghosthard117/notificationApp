...
markAllAsReadElement = document.querySelector('.header__link'),
unreadCountElement = document.querySelector('.unread-count')
...
markAllAsReadElement.addEventListener('click', () => {
  ...
  resetUnreadCount()
  removeNotifications()
  showNotifications(notificationCollection)
})
...
// Reset the count of unread to 0
...
function composeNotification(notification) {
  ...
  let titleContent = `<strong>${notification.name}</strong> 
  ${notification.content}`
  if (notification.type === 'link') {
    titleContent += ` <a href="#" class="link link--${notification.meta.linkType}">
      ${notification.meta.linkText}
    </a>`
  }
  if (notification.unread) {
...
// Show all notifications in the collection on screen
showNotifications(notificationCollection)
